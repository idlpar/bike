...
<body class="d-flex flex-column">
    <main class="d-flex justify-content-around align-items-center flex-grow-1">
        <div class="container-login container-fluid d-flex flex-column flex-md-row shadow rounded m-3 p-4 p-md-0">
            <div class="box-logo d-flex flex-column justify-content-center align-items-center px-4 pb-3 p-md-4">
                ...
                <?php else: ?>
                    <img class="logo w-100" src="<?= base_url('images/logo.svg') ?>" alt="<?= lang('software_short') . '&nbsp;' . $config['company'] ?>">
                <?php endif; ?>
            </div>
            <section class="box-login d-flex flex-column justify-content-center align-items-center p-md-4">
                <?= form_open('login') ?>
                <h3 class="text-center m-0 text-white"><?= lang('Login.welcome', [$config['company']]) ?></h3>
                <?php if ($has_errors): ?>
                ...
                <?php if (empty($config['login_form']) || 'floating_labels' === ($config['login_form'])): ?>
                    <div class="form-floating mt-3">
                        <input class="form-control form-control-lg" id="input-username" name="username" type="text" <?php if (ENVIRONMENT === 'testing') {
                            echo 'value="admin"';
                        } ?>>
                        <label for="input-username" class="text-white"><?= lang('Login.username') ?></label>
                    </div>
                    <div class="form-floating mb-3">
                        <input class="form-control form-control-lg" id="input-password" name="password" type="password" <?php if (ENVIRONMENT === 'testing') {
                ...
                <?php elseif ('input_groups' === ($config['login_form'])): ?>
                    ...
                    </div>box-logo d-flex flex-column justify-content-center align-items-center border-end border-secondary-subtle px-4 pb-3 p-md-4
                <?php endif; ?>
                <?php
                if ($gcaptcha_enabled) {
...
?>
    ...
    <footer class="d-flex justify-content-center flex-shrink-0 text-center">
        <div class="footer container-fluid rounded shadow p-3 mb-md-4 mx-md-3 bg-transparent border-0">
            <span class="text-primary">
                 <img class="logo w-100" src="<?= base_url('images/logo.svg') ?>" alt="<?= lang('software_short') . '&nbsp;' . $config['company'] ?>">
            </span>
<!--            <span>--><?php // = lang('Common.software_title')?><!--</span>-->
        </div>
    </footer>
</body>
